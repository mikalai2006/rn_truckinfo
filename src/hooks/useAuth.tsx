...
export default function useAuth() {
    ...
    const isTokenExpired = () => {
        ...
        const isExp = timeExp - time <= 0;

        // console.log('isTokenExpired: ', isExp, timeExp - time);
        return isExp;
    };

    const onSyncToken = async (): Promise<ITokens | null> => {
        try {
            const {access_token, refresh_token} = tokenFromStore || {};

            let isAuthActually = access_token && refresh_token && !isTokenExpired();
            const mayByRefresh = refresh_token && isTokenExpired();
    ...
    const onRefreshToken = async (): Promise<ITokens | null> => {
